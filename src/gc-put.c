...
MDBX_MAYBE_UNUSED static inline const char *dbg_prefix(gcu_t *ctx) {
  return ctx->lifo ? "    lifo" : "    fifo";
}
...
int gc_update(MDBX_txn *txn, gcu_t *ctx) {
  ...
  pgno_t prev_first_unallocated = 0;
...
retry_clean_adj:
  ctx->reserve_adj = 0;
retry:
  ctx->loop += prev_first_unallocated == txn->geo.first_unallocated;
  prev_first_unallocated = txn->geo.first_unallocated;
  ...
  while (true) {
    ...
    if (ctx->retired_stored != MDBX_PNL_GETSIZE(txn->tw.retired_pages) &&
        (ctx->loop == 1 || ctx->retired_stored > env->maxgc_large1page ||
         MDBX_PNL_GETSIZE(txn->tw.retired_pages) > env->maxgc_large1page)) {
    ...
    if (ctx->lifo) {
      ...
      while (ctx->cleaned_id <= txn->tw.gc.last_reclaimed) {
  ...
  if (ctx->amount) {
    ...
    if (txn->tw.gc.reclaimed == nullptr) {
      ...
      if (unlikely(rc != MDBX_SUCCESS)) {
        if (rc == MDBX_NOTFOUND && ctx->reserve_adj)
